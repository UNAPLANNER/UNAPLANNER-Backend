...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // Requirement
        modelBuilder.Entity<Requirement>()
            .HasCheckConstraint("CK_Requirement_Type", 
                "RequirementType IN ('Prerequisite', 'Corequisite')");
        modelBuilder.Entity<Requirement>()
            .HasCheckConstraint("CK_Requirement_NoSelfCycle", 
                "CourseId <> RequiredCourseId");

        // StudentProgress
        modelBuilder.Entity<StudentProgress>()
            .HasCheckConstraint("CK_StudentProgress_Status",
                "Status IN ('Pendiente', 'EnCurso', 'Aprobado', 'Reprobado')");
        modelBuilder.Entity<StudentProgress>()
            .HasCheckConstraint("CK_StudentProgress_Grade",
                "FinalGrade IS NULL OR (FinalGrade >= 0 AND FinalGrade <= 100)");
        modelBuilder.Entity<StudentProgress>()
            .HasCheckConstraint("CK_StudentProgress_Term",
                "AcademicTerm IS NULL OR AcademicTerm IN (1, 2)");

        // StudyPlanCourse
        modelBuilder.Entity<StudyPlanCourse>()
            .HasCheckConstraint("CK_StudyPlanCourse_Levels",
                "Levels BETWEEN 1 AND 6");
        modelBuilder.Entity<StudyPlanCourse>()
            .HasCheckConstraint("CK_StudyPlanCourse_Term",
                "Term IN (1, 2)");

        // Evaluation
        modelBuilder.Entity<Evaluation>()
            .HasCheckConstraint("CK_Evaluation_Percentage",
                "Percentage >= 0.01 AND Percentage <= 100");
        modelBuilder.Entity<Evaluation>()
            .HasCheckConstraint("CK_Evaluation_Score",
                "Score IS NULL OR (Score >= 0 AND Score <= 100)");
        modelBuilder.Entity<Evaluation>()
            .HasCheckConstraint("CK_Evaluation_Type",
                "EvaluationType IN ('Examen', 'Tarea', 'Proyecto', 'Quiz', 'Exposicion', 'Otro')");

        // Calendar
        modelBuilder.Entity<Calendar>()
            .HasCheckConstraint("CK_Calendar_Type",
                "ActivityType IN ('Examen', 'Tarea', 'Proyecto', 'Exposicion', 'Evento', 'Otro')");
        modelBuilder.Entity<Calendar>()
            .HasCheckConstraint("CK_Calendar_Reminder",
                "HasReminder = 0 OR ReminderDate IS NOT NULL");

        // File
        modelBuilder.Entity<UNAPLANNER_API.Models.Entities.File>()
            .HasCheckConstraint("CK_File_Type",
                "FileType IN ('PDF', 'JPG', 'PNG', 'DOCX', 'XLSX', 'TXT', 'OTRO')");

        // Notification
        modelBuilder.Entity<Notification>()
            .HasCheckConstraint("CK_Notification_Type",
